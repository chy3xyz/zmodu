...
fn generateAgentsMd(allocator: std.mem.Allocator, project_name: []const u8) ![]const u8 {
    ...
    try buf.appendSlice(allocator,
        ...
        \\### 3. AI writes ONLY in:
        \\```
        \\src/modules/<name>/ext/service.zig   # custom business logic
        \\src/modules/<name>/ext/api.zig       # custom API endpoints
        ...
        \\- service changes → write in `ext/service.zig`
        \\- api changes → write in `ext/api.zig`
        \\
        \\## File Markers
        \\
        \\Every generated file starts with `//! ⛔ @generated by zmodu — DO NOT EDIT`.
        \\If you see this marker, **stop — do not edit this file**.
        \\
        ...
        \\├── service.zig      # CRUD + validation (⛔)
        \\├── api.zig          # REST routes + handlers (⛔)
        \\└── ext/             # ✅ AI writes here
        \\    ├── service.zig   # custom business methods
        \\    └── api.zig       # custom endpoints
        \\```
        \\
        \\Subsystem modules (auto-detected from table prefixes like shop_*, crm_*):
        \\```
        \\src/modules/<subsystem>/<name>/
        \\├── module.zig       # ⛔ (same structure as above)
        \\├── ... 5 files ...
        \\└── ext/             # ✅
        \\```
        \\
...
fn cmdScaffold(io: std.Io, allocator: std.mem.Allocator, args: []const []const u8) !void {
    ...
    for (module_names.items) |mod_name| {
        ...
        // Generate ext/ subdirectory and ext/service.zig template per module
...
fn generateAiChatModule(io: std.Io, allocator: std.mem.Allocator, project_dir: []const u8, gen_opts: GenOptions) !void {
    ...
    try writeFileGen(io, prov_path,
        ...
        \\        const body = try self.buildBody(messages); defer self.allocator.free(body);
        \\        return body; // Stub: returns JSON body. Override in ext/service.zig for real HTTP call.
        \\    }
        \\    pub fn buildBody(self: *AiProvider, messages: []const ChatMsg) ![]const u8 {
        \\        var buf = std.ArrayList(u8).empty;
        \\        try buf.appendSlice(self.allocator, "{\"model\":\"");
        \\        try buf.appendSlice(self.allocator, self.model);
        \\        try buf.appendSlice(self.allocator, "\",\"messages\":[");
        \\        for (messages, 0..) |m, i| {
        \\            if (i > 0) try buf.appendSlice(self.allocator, ",");
        \\            try buf.appendSlice(self.allocator, "{\"role\":\"");
        \\            try buf.appendSlice(self.allocator, m.role);
        \\            try buf.appendSlice(self.allocator, "\",\"content\":\"");
        \\            try buf.appendSlice(self.allocator, m.content);
        \\            try buf.appendSlice(self.allocator, "\"}");
        \\        }
        \\        try buf.appendSlice(self.allocator, "]}");
        \\        return buf.toOwnedSlice(self.allocator);
        \\    }
        \\    pub fn extractContent(self: *AiProvider, resp: []const u8) ![]const u8 {
        \\        const tag = "\"content\":\"";
        \\        if (std.mem.indexOf(u8, resp, tag)) |start| {
        \\            const cs = start + tag.len;
        \\            if (std.mem.indexOf(u8, resp[cs..], "\"")) |end| return self.allocator.dupe(u8, resp[cs .. cs + end]);
        \\        }
        \\        return error.NoContentFound;
        \\    }
    ...
    try writeFileGen(io, svc_path,
        ...
        \\            const msgs = &[_]provider_mod.AiProvider.ChatMsg{.{ .role = "user", .content = content }};
        \\            const reply = try prov.chat(msgs);
        \\            defer prov.allocator.free(reply);
        \\            const saved = try msg_repo.insert(.{ .id = null, .conversation_id = conv_id, .role = "assistant", .content = reply, .created_at = 0 });
        ...
        \\        const list = std.ArrayList(provider_mod.AiProvider.ChatMsg).empty;
        \\        _ = conv_id;
        \\        _ = self;
        \\        return list;
        \\    }
...
fn generateClaudeSkills(io: std.Io, allocator: std.mem.Allocator, out_dir: []const u8, gen_opts: GenOptions) !void {
    ...
    try writeFileGen(io, so,
        ...
        \\
        \\## Regeneration Safety
        \\Custom logic in `ext/service.zig` and `ext/api.zig` survives regeneration.
...
fn generateScaffoldMainZig(allocator: std.mem.Allocator, project_name: []const u8, module_names: []const []const u8, sopts: ScaffoldOpts) ![]const u8 {
    ...
    try buf.appendSlice(allocator,
        \\//! ⛔ @generated by zmodu — DO NOT EDIT
        \\//! ✅ Add business logic in: src/business/, src/compat/
        \\//! ✅ Add custom routes in: src/modules/<name>/ext/api.zig
        \\
        \\const std = @import("std");
        \\const zigmodu = @import("zigmodu");
        \\
